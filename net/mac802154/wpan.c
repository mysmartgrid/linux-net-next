...
{
	struct ieee802154_llsec_params *params = &priv->sec.params;
	u8 level;
	u8 keymode = params->out_key.mode;
	...
	hdr->sec.level = level;
	hdr->sec.key_id_mode = keymode;
	if (keymode == IEEE802154_SCF_KEY_SHORT_INDEX)
		hdr->sec.short_src = params->out_key.short_source;
	else if (keymode == IEEE802154_SCF_KEY_HW_INDEX)
...
mac802154_wpan_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (hdr.fc.security_enabled) {
		int res = mac802154_llsec_encrypt(&priv->sec, skb);
		if (res) {
		      ...
		      const struct ieee802154_hdr *hdr)
{
	...
	if (hdr->fc.security_enabled) {
		if (!sdata->sec.params.enabled) {
			pr_debug("interface has security disabled\n");
			return NET_RX_DROP;
		}

		if (mac802154_llsec_decrypt(&sdata->sec, skb)) {
			pr_debug("decryption failed\n");
			return NET_RX_DROP;
		}
	}
